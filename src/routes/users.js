...
router.use(requireAuth, requireStoreUser, requireOwner);
...
router.delete('/:id', asyncHandler(async (req, res) => {
  ...
  const { data: target } = await supabase.from('profiles').select('role, store_id').eq('id', req.params.id).single();
  if (!target || target.store_id !== req.storeId) throw httpError(404, 'User not found');
